...
```mermaid
flowchart TD
    A["源系统<br/>MES / SFC / SharePoint"] --> B["数据抽取<br/>PQ/Parquet/CSV"]
    B --> C["数据落地与存档<br/>原始区 Raw"]
    C --> D["数据清洗与规范化<br/>ETL"]
    D --> E["标准时间处理<br/>统一接口"]
    E --> F["汇总与指标口径<br/>聚合/业务规则"]
    F --> G["数据发布<br/>Power BI / 文件订阅"]
...
| 序号 | 问题 | 方案 |
| --- | --- | --- |
| 1 | 多源分散/不同来源的字段不一致 | 建立“统一配置中心+字典表”，规范字段、维度、主键<br/>统一产品/工序/设备等字典<br/>字段映射规则入库并版本化 |
| 2 | 术语与KPI定义不清晰 | 建立“数据平台说明书”，标准化数据口径<br/>建立口径定义与 DAX/ETL 对应关系表<br/>明确 KPI 字段的定义与计算公式 |
| 3 | 数据没审核、升版不受控 | 在清洗与计算环节加入错误识别与通知预警<br/>发布前执行一致性回放作为门禁<br/>报表中对超阈值进行醒目标识并自动发送报告<br/>关键配置集中托管、变更留痕可追溯 |
| 4 | 刷新缓慢，每次都是全量刷新 | 采用数据压缩存档与增量刷新<br/>在线下完成数据处理后提交至 Power BI 展示 |
...
```mermaid
flowchart LR
  P0["试点现状<br/>SA/SFC"] --> P1[统一配置与字典]
  P1 --> P2[KPI口径中心+对照表]
  P2 --> P3[一致性回放门禁]
  P3 --> P4[数据压缩存档+增量刷新]
  P4 --> P5["平台推广<br/>多部门指标复用"]
```

- P1：完成产品/工序/设备字典与字段映射规则；配置集中化。  
- P2：发布KPI口径中心，建立DAX与ETL口径对应。  
- P3：将“样本期回放”接入发布门禁，异常自动阻断并提供明细。  
- P4：落地数据压缩存档和增量刷新策略，实现线下数据处理后提交Power BI展示，形成运行手册与SLA。  
