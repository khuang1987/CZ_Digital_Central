let
    ...
    // 文件路径：publish/SFC_batch_report_latest.parquet
    // ============================================
    ...
    TargetFiles = Table.SelectRows(PublishFolderFiles, each
        Text.StartsWith([Name], "SFC_batch_report") and
        [Extension] = ".parquet"
    ),
    ...
    // 读取Parquet文件
    Source = if FileContent <> null then Parquet.Document(Binary.Buffer(FileContent)) else #table({"BatchNumber", "CFN", "Operation"}, {}),
    ...
    // 定义期望的列顺序（只包含实际存在的列）
    // SFC数据字段顺序，包含Material相关字段
    DesiredColumns = {"BatchNumber", "CFN", "MaterialNumber", "MaterialDescription", "ProductionOrder", "Operation", "Operation description", 
        "Group", "machine", "ResourceCode", "ResourceDescription", "ProductionArea",
        "StepInQuantity", "TrackOutQuantity", "ScrapQuantity",
        ...
        "PreviousBatchEndTime", "Date created", "PlantCode", "VSM", "Name"},
