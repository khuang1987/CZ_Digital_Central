...
def setup_logging(cfg: Dict[str, Any]) -> None:
    ...
    log_file = cfg.get("logging", {}).get("file", "logs/etl_sa.log")
...
def process_mes_data(df: pd.DataFrame, cfg: Dict[str, Any]) -> pd.DataFrame:
    ...
    if "Operation" in result.columns:
        result["Operation"] = result["Operation"].astype(str).str.strip().str.zfill(4)
    if "BatchNumber" in result.columns:
    ...
    if "Resource" in result.columns:
        ...
        logging.info(f"已从Resource字段提取资源代码，创建machine字段")
    else:
...
def merge_sfc_data(mes_df: pd.DataFrame, cfg: Dict[str, Any]) -> pd.DataFrame:
    ...
    # 获取SFC latest文件路径
    sfc_latest_file = cfg.get("source", {}).get("sfc_latest_file", "publish/SFC_处理后数据_latest.parquet")
...
def calculate_pt(row: pd.Series) -> Optional[float]:
    """
    计算PT(d) - 实际加工时间（在机床上的时间）
    
    逻辑：优先使用Checkin_SFC（从SFC数据合并），否则使用TrackInTime
    不扣除周末，不区分工序
    
    根据文档：PT(d) = (TrackOutTime - 开始时间) / 24
    ...
    # 确定开始时间：优先使用Checkin_SFC，否则使用TrackInTime
    start_time = None
    if pd.notna(checkin_sfc):
        start_time = checkin_sfc
    elif pd.notna(trackin):
    ...
    try:
        ...
        for date in sample_dates:
            if date in df.index:
                ...
                logging.info(f"日历表示例: {date.date()} 是否工作日={is_work}")
...
def calculate_completion_status(row: pd.Series, calendar_df: pd.DataFrame) -> Optional[str]:
    ...
    逻辑：比较PT和ST（都只考虑工作日）
    - PT需要扣除非工作日：从PreviousBatchEndTime到TrackOutTime的实际时间中，扣除非工作日
    - ST是理论时间，保持原值
    - PT（工作日） > ST + 8小时容差 + 换批时间(0.5小时) → Overdue
    - PT（工作日） <= ST + 8小时容差 + 换批时间(0.5小时) → OnTime
    """
    # 获取PT和ST
    ...
    # 计算PT扣除非工作日后的值（只考虑工作日）
    # PT是从PreviousBatchEndTime到TrackOutTime的实际时间
    ...
    if pd.isna(previous_batch_end) or pd.isna(trackout):
        ...
        # 计算从PreviousBatchEndTime到TrackOutTime之间的非工作日小时数
        ...
        if end_dt > start_dt:
            ...
            non_workday_hours = calculate_nonworkday_hours(start_dt, end_dt, calendar_df)
            # PT（工作日）= 总时间 - 非工作日时间
            pt_workday_hours = total_hours - non_workday_hours
        else:
            pt_workday_hours = pt * 24
    
    # ST转换为小时（ST是理论时间，不需要扣除非工作日）
    st_hours = st * 24
    
    # 容差和换批时间（小时）
    tolerance_and_changeover = tolerance_h + 0.5  # 8小时容差 + 0.5小时换批
    
    # 比较：PT（工作日） > ST + 容差+换批时间 → Overdue
...
def process_all_data(cfg: Dict[str, Any], force_full_refresh: bool = False) -> pd.DataFrame:
    ...
    if force_full_refresh:
        ...
        history_file = incr_cfg.get("history_file", "publish/MES_处理后数据_latest.parquet")
...
def update_etl_state(df: pd.DataFrame, state_file: str, cfg: Dict[str, Any]) -> None:
    ...
    missing_fields = [f for f in key_fields if f not in df.columns]
    if missing_fields:
        logging.warning(f"无法更新状态：缺少字段 {missing_fields}")
        return
    ...
    # 获取最新的TrackOutTime
    if "TrackOutTime" in df.columns:
        latest_time = df["TrackOutTime"].max()
        if pd.notna(latest_time):
            state["last_processed_time"] = latest_time.isoformat() if isinstance(latest_time, pd.Timestamp) else str(latest_time)
    ...
    logging.info(f"状态更新完成：已处理记录 {len(processed_hashes)} 条，本次新增 {len(new_hashes)} 条")
...
def save_to_parquet(df: pd.DataFrame, output_path: str, cfg: Dict[str, Any]) -> None:
    ...
    numeric_fields = [
        "StepInQuantity", "TrackOutQuantity", "ProductionOrder",
        "LT(d)", "PT(d)", "ST(d)", "NonWorkday(d)", "Tolerance(h)",
    ...
    for col in numeric_fields:
        if col in df.columns:
            ...
            if col in ["StepInQuantity", "TrackOutQuantity", "ProductionOrder", "Machine(#)"]:
                # 确保是Int64类型（支持NaN）
    ...
    for col in df.columns:
        if col not in datetime_fields and df[col].dtype == 'object':
    ...
    try:
        ...
        if mes_result_df.empty:
            ...
            if incr_cfg.get("enabled", False):
                ...
                latest_file = os.path.join(output_dir, "MES_处理后数据_latest.parquet")
