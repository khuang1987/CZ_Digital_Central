"""
SFC 产品检验原始数据 ETL 脚本 (V2 架构)
功能：从 Excel 读取 SFC 产品检验数据，简单清洗后存入数据库原始表
"""
...
# 配置
CONFIG_PATH = os.path.join(current_dir, "..", "config", "config_sfc_product_inspection.yaml")
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(LOG_DIR, f"etl_sfc_inspection_raw_{datetime.now().strftime('%Y%m%d')}.log"), encoding='utf-8'),
...
def read_inspection_files(
    ...
    """读取 SFC 产品检验 Excel 文件"""
    inspection_path = cfg.get("source", {}).get("product_inspection_path", "")
    if not inspection_path:
        logging.error("未找到 product_inspection_path 配置")
        return []
    
    db = get_db_manager()
    file_paths = glob.glob(inspection_path)
    # Normalize paths to absolute and standard format
    file_paths = [os.path.normpath(os.path.abspath(p)) for p in file_paths]
    
    if not file_paths:
        logging.warning(f"未找到匹配的文件: {inspection_path}")
        return []
    ...
    # 过滤未变化的文件
    changed_files = db.filter_changed_files("sfc_inspection_raw", file_paths)

    def _sort_key(p: str) -> float:
        try:
            dt = extract_report_date_from_filename(os.path.basename(p))
            return dt.timestamp()
        except Exception:
            try:
    ...
    changed_files = sorted(changed_files, key=_sort_key, reverse=True)
...
def clean_inspection_data(df: pd.DataFrame, cfg: Dict[str, Any]) -> pd.DataFrame:
    """简单清洗 SFC 产品检验数据"""
    if df.empty:
        return df
    ...
    # 时间字段转换
    if 'ReportDate' in result.columns:
        result['ReportDate'] = pd.to_datetime(result['ReportDate'], errors='coerce')
    ...
    # 生成 record_hash
    key_fields = ['BatchNumber', 'SerialNumber', 'Operation', 'ReportDate']
...
def save_to_database(df: pd.DataFrame, table_name: str = "raw_sfc_inspection") -> Dict[str, int]:
    """保存数据到数据库"""
    if df.empty:
...
def main(
    ...
    logging.info("=" * 60)
    logging.info("SFC 产品检验原始数据 ETL (V2) 启动")
    if test_mode:
    ...
    try:
        ...
        files_to_process = read_inspection_files(
            cfg,
            test_mode=test_mode,
            max_files=max_files,
            max_rows=max_rows,
        ...
        for file_path in files_to_process:
            try:
                ...
                df_file["ReportDate"] = extract_report_date_from_filename(os.path.basename(file_path))

                total_read += len(df_file)

                df_clean = clean_inspection_data(df_file, cfg)
                stats = save_to_database(df_clean, "raw_sfc_inspection")

                total_inserted += stats["inserted"]
                total_skipped += stats["skipped"]

                db.mark_file_processed("sfc_inspection_raw", file_path)
                logging.info(
        ...
        count = db.get_table_count("raw_sfc_inspection")
        logging.info(f"数据库 raw_sfc_inspection 表当前记录数: {count}")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='SFC 产品检验原始数据 ETL')
