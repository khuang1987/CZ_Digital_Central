...
# 配置路径
CONFIG_PATH = get_config_path("mes_batch_report", "mes", os.path.dirname(os.path.abspath(__file__)))
LOG_PATH = get_log_path("mes")
STATE_PATH = get_state_path("mes_batch_report", "mes", os.path.dirname(os.path.abspath(__file__)))
...
def process_mes_data(df: pd.DataFrame, cfg: Dict[str, Any]) -> pd.DataFrame:
    ...
    # 5. Operation 转为可空整数，BatchNumber 统一为文本
...
def merge_standard_time(mes_df: pd.DataFrame, cfg: Dict[str, Any]) -> pd.DataFrame:
    ...
    # 检查必要字段（至少需要Operation；Group可能缺失，不能因此直接跳过合并）
    ...
    # 兼容：标准时间表或MES数据缺少Group字段时，用空字符串占位
    if "Group" not in std_time_df.columns:
        std_time_df["Group"] = ""
    if "Group" not in mes_df.columns:
        mes_df["Group"] = ""

    # 统一合并键数据类型 + 关键字段清洗（trim / 大小写 / Excel尾部".0"）
    for col in ["CFN", "Operation", "Group"]:
        if col in mes_df.columns:
            mes_df[col] = mes_df[col].astype(str).str.strip().replace('nan', '').replace('None', '')
    for col in [c for c in ["CFN", "Material Number", "Operation", "Group"] if c in std_time_df.columns]:
        std_time_df[col] = std_time_df[col].astype(str).str.strip().replace('nan', '').replace('None', '')

    if "CFN" in mes_df.columns:
        mes_df["CFN"] = mes_df["CFN"].str.upper()
    if "CFN" in std_time_df.columns:
        std_time_df["CFN"] = std_time_df["CFN"].str.upper()

    if "Group" in mes_df.columns:
        mes_df["Group"] = mes_df["Group"].str.replace(r"\\.0$", "", regex=True)
    if "Group" in std_time_df.columns:
        std_time_df["Group"] = std_time_df["Group"].str.replace(r"\\.0$", "", regex=True)
    ...
    std_value_rename = {
        ...
        "Quantity": "_st_qty",
        "OEE": "_st_oee",
        "Setup Time (h)": "_st_setup_h",
    }

    result = mes_df.copy()

    # 第一步：优先使用 MES.CFN 匹配标准表 CFN（Operation+Group）
    if "CFN" in mes_df.columns and "CFN" in std_time_df.columns:
        std1_cols = ["CFN", "Operation", "Group"] + value_cols
        std1 = std_time_df[std1_cols].drop_duplicates(subset=["CFN", "Operation", "Group"]).rename(columns=std_value_rename)
        result = result.merge(std1, on=["CFN", "Operation", "Group"], how="left")
    else:
        for c in std_value_rename.values():
            result[c] = None
    
    # 计算单件时间（Machine和Labor除以Quantity，单位：秒）
    # 重命名为EH_machine(s)和EH_labor(s)
    if "_st_machine" in result.columns and "_st_qty" in result.columns:
        quantity = result["_st_qty"].fillna(1).replace(0, 1)
        result["EH_machine(s)"] = result["_st_machine"] / quantity
    else:
        result["EH_machine(s)"] = None
    
    if "_st_labor" in result.columns and "_st_qty" in result.columns:
        quantity = result["_st_qty"].fillna(1).replace(0, 1)
        result["EH_labor(s)"] = result["_st_labor"] / quantity
    else:
        result["EH_labor(s)"] = None
    
    if "_st_oee" in result.columns:
        result["OEE"] = result["_st_oee"]
    if "_st_setup_h" in result.columns:
        result["Setup Time (h)"] = result["_st_setup_h"]

    for c in ["_st_machine", "_st_labor", "_st_qty", "_st_oee", "_st_setup_h"]:
        if c in result.columns:
            result = result.drop(columns=[c])
    
    # 第二步：对未匹配的记录，使用 MES.CFN 匹配标准表 CFN（忽略Group）
    unmatched_mask = result["EH_machine(s)"].isna() & result["EH_labor(s)"].isna()
    unmatched_count = int(unmatched_mask.sum())

    if unmatched_count > 0 and "CFN" in mes_df.columns and "CFN" in std_time_df.columns:
        logging.info(f"第一步CFN匹配后，还有 {unmatched_count} 条记录未匹配，尝试忽略Group字段匹配")

        unmatched_df = result[unmatched_mask].copy()
        unmatched_df = unmatched_df.reset_index().rename(columns={"index": "_orig_index"})

        std_fb_cols = ["CFN", "Operation"] + value_cols
        std_fb = std_time_df[std_fb_cols].drop_duplicates(subset=["CFN", "Operation"]).rename(columns=std_value_rename)
        fb = unmatched_df.merge(std_fb, on=["CFN", "Operation"], how="left")

        if "_st_machine" in fb.columns and "_st_qty" in fb.columns:
            quantity = fb["_st_qty"].fillna(1).replace(0, 1)
            fb["EH_machine(s)"] = fb["_st_machine"] / quantity
        else:
            fb["EH_machine(s)"] = None

        if "_st_labor" in fb.columns and "_st_qty" in fb.columns:
            quantity = fb["_st_qty"].fillna(1).replace(0, 1)
            fb["EH_labor(s)"] = fb["_st_labor"] / quantity
        else:
            fb["EH_labor(s)"] = None

        if "_st_oee" in fb.columns:
            fb["OEE"] = fb["_st_oee"]
        if "_st_setup_h" in fb.columns:
            fb["Setup Time (h)"] = fb["_st_setup_h"]

        fb_matched_mask = fb["EH_machine(s)"].notna() | fb["EH_labor(s)"].notna()
        if fb_matched_mask.any() and "_orig_index" in fb.columns:
            matched_rows = fb[fb_matched_mask]
            for col in ["EH_machine(s)", "EH_labor(s)", "OEE", "Setup Time (h)"]:
                if col not in matched_rows.columns:
                    continue
                s = matched_rows[["_orig_index", col]].copy()
                s = s.dropna(subset=["_orig_index"])
                if s.empty:
                    continue
                result.loc[s["_orig_index"].astype(int).tolist(), col] = s[col].tolist()

            logging.info(f"忽略Group匹配成功: {len(matched_rows)} 条记录")

    # 第三步：对未匹配的记录，使用 MES.CFN 匹配标准表的 Material Number（Operation+Group）
    unmatched_mask = result["EH_machine(s)"].isna() & result["EH_labor(s)"].isna()
    unmatched_count = int(unmatched_mask.sum())
    
    if unmatched_count > 0 and "Material Number" in std_time_df.columns and "CFN" in mes_df.columns:
        logging.info(f"第一步CFN匹配后，还有 {unmatched_count} 条记录未匹配，尝试使用Material Number匹配")
        
        # 获取未匹配的记录
        unmatched_df = result[unmatched_mask].copy()
        # 保留原始索引用于回写（merge会重建index，不能直接用merge后的index回写到result）
        unmatched_df = unmatched_df.reset_index().rename(columns={"index": "_orig_index"})
        ...
        for i, mes_col in enumerate(merge_cols_mes):
            ...
            if mes_col in unmatched_df.columns:
                s = unmatched_df[mes_col].astype(str).str.strip().replace('nan', '').replace('None', '')
                if mes_col == "CFN":
                    s = s.str.upper()
                if mes_col == "Group":
                    s = s.str.replace(r"\\.0$", "", regex=True)
                unmatched_df[mes_col] = s
            if std_col in std_time_df.columns:
                s = std_time_df[std_col].astype(str).str.strip().replace('nan', '').replace('None', '')
                if std_col in ["Material Number", "CFN"]:
                    s = s.str.upper()
                if std_col == "Group":
                    s = s.str.replace(r"\\.0$", "", regex=True)
                std_time_df[std_col] = s
        
        std2_cols = ["Material Number", "Operation", "Group"] + value_cols
        std2 = std_time_df[std2_cols].rename(columns=std_value_rename)
        ...
        if "_st_machine" in cfn_matched.columns and "_st_qty" in cfn_matched.columns:
            quantity = cfn_matched["_st_qty"].fillna(1).replace(0, 1)
            cfn_matched["EH_machine(s)"] = cfn_matched["_st_machine"] / quantity
        else:
            cfn_matched["EH_machine(s)"] = None

        if "_st_labor" in cfn_matched.columns and "_st_qty" in cfn_matched.columns:
            quantity = cfn_matched["_st_qty"].fillna(1).replace(0, 1)
            cfn_matched["EH_labor(s)"] = cfn_matched["_st_labor"] / quantity
        else:
            cfn_matched["EH_labor(s)"] = None

        if "_st_oee" in cfn_matched.columns:
            cfn_matched["OEE"] = cfn_matched["_st_oee"]
        if "_st_setup_h" in cfn_matched.columns:
            cfn_matched["Setup Time (h)"] = cfn_matched["_st_setup_h"]

        for c in ["_st_machine", "_st_labor", "_st_qty", "_st_oee", "_st_setup_h", "Material Number"]:
            if c in cfn_matched.columns:
                cfn_matched = cfn_matched.drop(columns=[c])
        ...
        if cfn_matched_mask.any():
            matched_rows = cfn_matched[cfn_matched_mask]
            if "_orig_index" not in matched_rows.columns:
                logging.warning("Material Number匹配结果缺少_orig_index，无法回写到原表")
            else:
                # 更新result中对应索引的记录（按_orig_index回写）
                for col in ["EH_machine(s)", "EH_labor(s)", "OEE", "Setup Time (h)"]:
                    if col not in matched_rows.columns:
                        continue
                    s = matched_rows[["_orig_index", col]].copy()
                    s = s.dropna(subset=["_orig_index"])  # 安全保护
                    if s.empty:
                        continue
                    result.loc[s["_orig_index"].astype(int).tolist(), col] = s[col].tolist()

            matched_count = len(matched_rows)
            logging.info(f"第二步Material Number匹配成功: {matched_count} 条记录")
...
def process_all_data(cfg: Dict[str, Any], force_full_refresh: bool = False) -> tuple:
    """
    处理所有数据的主函数
    
    Returns:
        tuple: (DataFrame, IncrementalProcessor) - 处理后的数据和文件级增量处理器
    """
    # 检查测试模式配置
    ...
    # 初始化文件级增量处理器（状态文件统一放到项目目录）
    state_dir = os.path.join(project_root, "data_pipelines", "sources", "mes", "state")
    os.makedirs(state_dir, exist_ok=True)
    file_state_file = os.path.join(state_dir, "etl_mes_files_state.json")
    
    file_incr_processor = IncrementalProcessor(
        state_file=file_state_file,
        unique_key_fields=["BatchNumber", "Operation", "machine", "TrackOutTime"]
    )
    ...
    if mes_sources:
        ...
        # 第1层：文件级去重 - 检查所有工厂文件是否有变化
        # 支持 pattern（通配符）和 path（单文件）两种配置方式
        all_files = []
        for s in mes_sources:
            ...
            if s.get("pattern"):
                # 通配符模式：展开匹配的所有文件
                matched = glob.glob(s["pattern"])
                all_files.extend(matched)
            elif s.get("path"):
                # 单文件模式
                all_files.append(s["path"])
        
        changed_files = file_incr_processor.filter_changed_files(all_files)
        
        if not changed_files and not force_full_refresh:
            logging.info("所有MES数据文件未变化，跳过处理")
            return pd.DataFrame(), file_incr_processor
    ...
    if force_full_refresh:
        ...
        try:
            db = get_default_db_manager()
            db.execute_sql('DROP TABLE IF EXISTS mes_batch_report_latest')
            logging.info("已清除SQLite历史表: mes_batch_report_latest")
        except Exception as e:
    ...
    if mes_sources:
        for source in mes_sources:
            ...
            if source.get("pattern"):
                # 通配符模式：标记所有匹配的文件
                matched = glob.glob(source["pattern"])
                for f in matched:
                    file_incr_processor.mark_file_processed(f)
            elif source.get("path"):
                # 单文件模式
                file_incr_processor.mark_file_processed(source["path"])
    
    # 返回MES结果和文件级增量处理器
    return result, file_incr_processor


def generate_record_hash(row: pd.Series, key_fields: List[str]) -> str:
    """生成记录的唯一标识（与update_etl_state保持一致）"""
    # 提取关键字段的值，使用简单字符串拼接（与update_etl_state一致）
    values = []
...
def filter_incremental_data(df: pd.DataFrame, cfg: Dict[str, Any], state_file: str) -> pd.DataFrame:
    ...
    # 兼容两种字段名：processed_hashes 和 processed_records
    processed_hashes = set(state.get("processed_hashes", state.get("processed_records", [])))
    ...
    # 生成每条记录的hash并筛选新数据（使用向量化操作，避免apply）
...
def merge_with_history(new_df: pd.DataFrame, history_file: str, cfg: Dict[str, Any]) -> pd.DataFrame:
    ...
    if new_df.empty:
        ...
        try:
            db = get_default_db_manager()
            with db.get_connection() as conn:
                history_df = pd.read_sql_query('SELECT * FROM "mes_batch_report_latest"', conn)
            return history_df
        except Exception as e:
    ...
    try:
        ...
        with db.get_connection() as conn:
            history_df = pd.read_sql_query('SELECT * FROM "mes_batch_report_latest"', conn)
        logging.info(f"加载SQLite历史数据: {len(history_df)} 行")
        ...
        if available_key_fields:
            ...
            try:
                db.bulk_insert(combined_df, 'mes_batch_report_latest', if_exists='replace')
            except Exception as e:
...
if __name__ == "__main__":
    ...
    try:
        # 处理MES数据
        mes_result_df, file_incr_processor = process_all_data(cfg, force_full_refresh=force_full_refresh)
        ...
        if mes_result_df.empty:
            ...
            if WRITE_PARQUET_OUTPUT:
                ...
                # 本地项目目录
                local_publish_dir = os.path.join(project_root, "data_pipelines", "sources", "mes", "publish")
                ...
                if incr_cfg.get("enabled", False):
                    ...
                    history_file = incr_cfg.get("history_file", "publish/MES_batch_report_latest.parquet")
                    history_file = os.path.join(BASE_DIR, history_file) if not os.path.isabs(history_file) else history_file
                    local_file = os.path.join(local_publish_dir, "MES_batch_report_latest.parquet")
                    save_to_dual_locations(mes_result_df, history_file, local_file, cfg)
                    
                    # 更新状态文件（统一放到项目目录）
                    state_dir = os.path.join(project_root, "data_pipelines", "sources", "mes", "state")
                    state_file = os.path.join(state_dir, "etl_mes_records_state.json")
                    update_etl_state(mes_result_df, state_file, cfg)
                    
                    logging.info("增量处理：已保存到双份位置并更新状态")
                else:
                    # 如果未启用增量处理，也保存到双份位置
                    latest_file = os.path.join(output_dir, "MES_batch_report_latest.parquet")
                    local_file = os.path.join(local_publish_dir, "MES_batch_report_latest.parquet")
                    save_to_dual_locations(mes_result_df, latest_file, local_file, cfg)
                    logging.info(f"MES数据已保存到双份位置")

            # 保存文件级增量状态
            file_incr_processor.save()

            # ============================================================
            # 写入SQLite数据库（供后续统一导出Parquet使用）
            # ============================================================
            try:
                logging.info("开始写入数据库...")
                db = get_default_db_manager()
                inserted_count = db.bulk_insert(mes_result_df, 'mes_batch_report_latest', if_exists='replace')
                logging.info(f"数据库写入完成: 共 {inserted_count} 条记录")

                db.log_etl_run(
                    etl_name='mes_batch_report',
                    status='success',
                    records_read=len(mes_result_df),
                ...
                logging.warning(f"数据库写入失败: {db_error}")
