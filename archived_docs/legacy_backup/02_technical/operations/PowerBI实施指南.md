...
## 方案架构
```
SQL Server → export_core_to_a1.py → Parquet/TSV → Power BI → Power Automate → Planner
```
...
2. 点击"获取数据" → "Parquet"
3. 选择文件：`A1_ETL_Output/03_METADATA/monitoring/kpi_trigger_results.parquet`
4. （可选）重复步骤导入：
   - `A1_ETL_Output/03_METADATA/monitoring/kpi_global_matrix.csv`
   - `A1_ETL_Output/03_METADATA/monitoring/pareto_top_matrix.csv`
...
2. **Get file content**（OneDrive/SharePoint，指向 `A1_ETL_Output/03_METADATA/monitoring/kpi_trigger_results.tsv`）
...
### 4.1 方案A：Windows 任务计划（推荐）

目标：每天先刷新 SQL Server + 生成 `A1_ETL_Output` 的 Parquet/TSV，再由 Power BI/Power Automate 消费。

推荐运行脚本：

- 日常：`scripts/refresh_core_data.bat`
- 低频全量（周/月）：`scripts/refresh_all_data.bat`
...
4. 触发器：每天 07:00
5. 操作：启动程序
   - 程序：`scripts/refresh_core_data.bat`
   - 起始位置：项目根目录
...
1. **Python脚本测试**
   ```bash
   python "03_Scripts/trigger_export.py"
   ```
